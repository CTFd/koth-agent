...
func main() {
	...
	if health_cmd == "" {
		flag.StringVar(&health_cmd, "health-cmd", "true", "Specify a command to run when asked for a healthcheck")
	}

	if owner_cmd == "" {
		flag.StringVar(&owner_cmd, "owner-cmd", "", "Specify a command to run when asked for an owner")
	}
